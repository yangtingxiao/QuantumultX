/*
京东疯狂的Joy
更新时间：2020-12-05 14:17
脚本说明：
可自动签到，看视频，生产金币，领金币宝箱，做任务，合并（34级不合并），自定义购买等级，互助
可以到BoxJs中开启相应功能
cron自己看着设置，如开启生产金币则会24小时执行
因生产金币需要24小时执行，不建议在手机上开启，因此多账号也只能执行第一个账号，需要多账号的多复制几份分别执行即可
如果手机上开启了收金币，无法停止执行的，请重启VPN
...
const $ = new Env('京东疯狂的Joy');
//Node.js用户请在jdCookie.js处填写京东ck;
const jdCookieNode = $.isNode() ? require('./jdCookie.js') : '';
const STRSPLIT = "|";       //分隔符
const printDetail = $.getdata("CFG_CRAZYJOY_LOG") ? $.getdata("CFG_CRAZYJOY_LOG") === "true" : false //是否显示出参详情
...
const JD_API_HOST = `https://api.m.jd.com/?appid=crazy_joy`;
!(async () => {
  if (!cookiesArr[0]) {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    if (cookie) {
      ...
      await Promise.all([
        crazyJoy_user_gameState(),
        crazyJoy_task_recordAssist(),
        crazyJoy_task_getTaskState(),
        crazyJoy_task_doSign(),
        crazyJoy_joy_produce()
      ])
      //await msgShow();
    }
  }
...
//看视频
function crazyJoy_event_getVideoAdvert(eventType,eventRecordId,timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      let body = {eventType : eventType, eventRecordId : eventRecordId}
      let url = getUrl("crazyJoy_event_getVideoAdvert",body)
      $.get(url, async (err, resp, data) => {
        try {
          if (printDetail) console.log(data)
          data = JSON.parse(data);
          crazyJoy_event_obtainAward(eventType,eventRecordId,40000);
        } catch (e) {
          $.logErr(e, resp);
        } finally {
          resolve()
...
//领奖励
function crazyJoy_event_obtainAward(eventType,eventRecordId,timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      let body = {}
      if (eventType) body.eventType = eventType;
      if (eventRecordId) body.eventRecordId = eventRecordId;
      let url = getUrl("crazyJoy_event_obtainAward",body)
      $.get(url, async (err, resp, data) => {
        try {
          if (printDetail) console.log(data)
          data = JSON.parse(data);
          console.log(`当前宝箱类型：${eventType}`)
...
//获取首页信息
function crazyJoy_user_gameState(timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      let body = {paramData : {"inviter":"7FtFH4sbCqEAfYJCePLeTqt9zd5YaBeE"}}
      let url = getUrl("crazyJoy_user_gameState",body)
      $.get(url, async (err, resp, data) => {
        try {
          if (printDetail) console.log(data)
          //console.log(data)
          data = JSON.parse(data);
          if (!data.data) {
            console.log(`请求出现错误：${data.message}`)
            await $.wait(2000)//异常 休息一下
            if (data.resultCode !== "1000009" && autoProduce) await crazyJoy_user_gameState()
            return
          }
          console.log(`您的助力码为：${data.data.userInviteCode}`)
          userTopLevelJoyId = data.data.userTopLevelJoyId; //获取最大joyId
          await crazyJoy_joy_allowBoughtList()
          needToBought = Math.pow(2,userTopLevelJoyId - (setBoughtJoyId||userCanBoughtTopLevelJoyId))
          console.log(`当前最大等级：${userTopLevelJoyId}级\n升级需要个数：${needToBought}个${setBoughtJoyId||userCanBoughtTopLevelJoyId}级`)
          ...
          for (let i in joyIds){
            ...
            if (autoSellJoy && joyIds[i] && (joyIds[i] < (setBoughtJoyId||userCanBoughtTopLevelJoyId))) {
              await crazyJoy_joy_trade("SELL",joyIds[i],i)
              boxCount++
            }
          }
          ...
          if (boxCount&&!lackCoin&&autoBuyJoy)
            ...
            await crazyJoy_user_gameState(10 * 60 * 1000);//10分钟执行一次
        } catch (e) {
          $.logErr(e, resp);
        } finally {
          resolve()
...
//获取可购买joy
function crazyJoy_joy_allowBoughtList(timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      let body = {paramData : ""}
      body.paramData = {"entry":"SHOP"}
      let url = getUrl("crazyJoy_joy_allowBoughtList",body)
      $.get(url, async (err, resp, data) => {
        try {
          if (printDetail) console.log(data)
          data = JSON.parse(data);
          if (data.resultCode !== "0") {
            ...
            await crazyJoy_joy_allowBoughtList(timeout)
            return
          }
          userCanBoughtTopLevelJoyId = data.data.shop[data.data.shop.length - 2].joyId; //获取最大可购买joyId
          console.log(`最大购买等级：${userCanBoughtTopLevelJoyId}级`)
          if (setBoughtJoyId && autoBuyJoy) {
            console.log(`设定购买等级：${setBoughtJoyId}级`)
            let suggestBought = ""
            for (let i = setBoughtJoyId - 1;i < data.data.shop.length - 2;i++) {
              if (setBoughtJoyId === 30) break;
              if (data.data.shop[i].coins * 2 > data.data.shop[i + 1].coins) {
                //setBoughtJoyId++
                suggestBought = `更换购买等级：设定的比下一级花费更多，建议更换为${i + 2}级`
                //console.log(`更换购买等级：设定的比下一级花费更多，建议更换为${i + 2}级`)
              } else break
            }
            if (suggestBought) console.log(suggestBought)
          }
          if (setBoughtJoyId > userCanBoughtTopLevelJoyId || setBoughtJoyId > 30) setBoughtJoyId = 0
          console.log(`购买需要金币：${addUnit(data.data.shop[(setBoughtJoyId||userCanBoughtTopLevelJoyId) - 1].coins)}`)
        } catch (e) {
          $.logErr(e, resp);
        } finally {
          resolve()
...
//合并joy
function crazyJoy_joy_moveOrMerge(fromBoxIndex,targetBoxIndex,timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      let body = {operateType : "MERGE",fromBoxIndex : fromBoxIndex,targetBoxIndex: targetBoxIndex}
      let url = getUrl("crazyJoy_joy_moveOrMerge",body)
      $.get(url, async (err, resp, data) => {
        try {
          if (printDetail) console.log(data)
          data = JSON.parse(data);
...
//收金币
function crazyJoy_joy_produce(timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      let url = getUrl("crazyJoy_joy_produce",{})
      $.get(url, async (err, resp, data) => {
        try {
          if (printDetail) console.log(data)
          data = JSON.parse(data);
          if (data.data&&data.data.totalCoinAmount) console.log(`当前金币数量：${addUnit(data.data.totalCoinAmount)}`)
          if (data.data&&data.data.luckyBoxRecordId && autoProduce &&!luckyBoxArr.includes(data.data.luckyBoxRecordId)) {
            //console.log(JSON.stringify(data))
            luckyBoxArr.push(data.data.luckyBoxRecordId)
            console.log(`开始观看视频：剩余观看次数${data.data.advertViewTimes}次`)
            crazyJoy_event_getVideoAdvert("LUCKY_BOX_DROP",data.data.luckyBoxRecordId)
          }
          if (autoProduce) await crazyJoy_joy_produce(4000)
        } catch (e) {
          $.logErr(e, resp);
        } finally {
          resolve()
...
//签到
function crazyJoy_task_doSign(timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      let url = getUrl("crazyJoy_task_doSign",{})
      $.get(url, async (err, resp, data) => {
        try {
          if (printDetail) console.log(data)
          data = JSON.parse(data);
          //console.log('开始签到')
          if (!data.success) console.log(`今日签到失败：${data.message}`)
          if (data.data&&parseInt(data.data.coins)) console.log(`签到获得金币：${data.data.coins}`)
          if (data.data&&data.data.beans) console.log(`签到获得京豆：${data.data.beans}`)
          if (data.data&&data.data.joyId) console.log(`签到获得疯狗：${data.data.joyId}级`)
        } catch (e) {
          $.logErr(e, resp);
        } finally {
          resolve()
...
//查询任务
function crazyJoy_task_getTaskState(timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      let body = {paramData : ""}
      body.paramData = {"taskType":"DAY_TASK"}
      let url = getUrl("crazyJoy_task_getTaskState",body)
      $.get(url, async (err, resp, data) => {
        try {
          if (printDetail) console.log(data)
          //console.log(data)
          ...
          for (let i in data.data) {
             if (data.data[i].status === 0 && data.data[i].taskTypeId !== 102)  {
               await crazyJoy_task_viewPage(data.data[i].taskId,data.data[i].taskRecordId)
             } else if (data.data[i].status === 2) {
               await crazyJoy_task_obtainAward(data.data[i].taskId)
             }
          }
        } catch (e) {
          $.logErr(e, resp);
...
//做任务
function crazyJoy_task_viewPage(taskId,taskRecordId,timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      let body = {"action":"MARK","taskId":taskId}
      let url = getUrl("crazyJoy_task_viewPage",body)
      $.get(url, async (err, resp, data) => {
        try {
          if (printDetail) console.log(data)
          //console.log(data)
          data = JSON.parse(data);
          console.log(`开始每日任务：${data.data&&data.data.taskTitle||data.message}`)
          if (data.data) await crazyJoy_task_viewPage_app(taskId,taskRecordId,40000)
        } catch (e) {
          $.logErr(e, resp);
        } finally {
          resolve()
...
//做任务
function crazyJoy_task_viewPage_app(taskId,taskRecordId,timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      let body = {"taskId":taskId,"taskRecordId":taskRecordId,"rechargeversion":"8.3","action":"INCREASE"}
      let url = getUrl("crazyJoy_task_viewPage",body)
      $.get(url, async (err, resp, data) => {
        try {
          if (printDetail) console.log(data)
          data = JSON.parse(data);
          console.log(`每日任务结束：${data.data.taskTitle}`)
          await crazyJoy_task_obtainAward(taskId,1000)
        } catch (e) {
          $.logErr(e, resp);
        } finally {
          resolve()
...
//完成任务
function crazyJoy_task_obtainAward(taskId,timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      let body = {"taskId":taskId}
      let url = getUrl("crazyJoy_task_obtainAward",body)
      $.get(url, async (err, resp, data) => {
        try {
          if (printDetail) console.log(data)
          data = JSON.parse(data);
          console.log(`任务获得金币：${addUnit(data.data.coins)}`)
        } catch (e) {
          $.logErr(e, resp);
        } finally {
          resolve()
...
//助力
function crazyJoy_task_recordAssist(inviter = "",timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      let body = {"paramData":{"inviter": inviter||"7FtFH4sbCqEAfYJCePLeTqt9zd5YaBeE"}}
      let url = getUrl("crazyJoy_task_recordAssist",body)
      $.get(url, async (err, resp, data) => {
        try {
          if (printDetail) console.log(data)
          ...
          if (data.resultCode === "0") {
            ...
            if (inviter) console.log(`助力执行失败：${data.message}`)
          }
          if (inviter === "") {
...
//joy交易
function crazyJoy_joy_trade(action,joyId,boxId,timeout = 0) {
  return new Promise((resolve) => {
    setTimeout( ()=>{
      let body = {}
      if (action) body.action = action;
      if (joyId) body.joyId = joyId;
      if (boxId) body.boxId = boxId;
      let url = getUrl("crazyJoy_joy_trade",body)
      $.get(url, async (err, resp, data) => {
        try {
          if (printDetail) console.log(data)
          //console.log(data)
          data = JSON.parse(data);
          if (action === "BUY") {
            if (data.data&&data.data.lackCoin) {
              lackCoin = true;
              console.log(`购买请求失败：${joyId}级，金币不足`)
              return
            } else if (!data.data) {
              console.log(JSON.stringify(data))
              console.log(`购买请求失败：结果未知`)
              return
            }
            console.log(`购买请求成功：${joyId}级，格子${data.data.boxId + 1}`)
            joyIds[data.data.boxId] = joyId
          } else {
            console.log(`回收请求成功：${joyId}级`)
            joyIds[boxId] = 0;
          }
        } catch (e) {
          $.logErr(e, resp);
...
function getUrl(functionId,body = "") {
  let  url = {url : `${JD_API_HOST}&body=${encodeURI(JSON.stringify(body))}&uts=${get_uts(body)}&functionId=${functionId}&t=${Date.now()}`,
    headers : {
  ...
  //if (printDetail) console.log(url.url)
  return url;
}

function get_uts(body) {
  let t= Date.now().toString().substr(0, 10)
  return $.CryptoJS.MD5("aDvScBv$gGQvrXfva8dG!ZC@DA70Y%lX" + (JSON.stringify(body) || "") + t).toString() +  Number(t).toString(16)
}

function addUnit(money) {
  let unitList=["K","M","B","T","Q"]
  if (money < 1000) return money
  for (let i in unitList) {
    money = (money / 1000).toFixed(1)
    if (money < 1000) return money + unitList[i]
  }
  if (money >= 1000) return money + unitList[unitList.length -1]
}
...
function msgShow() {
  let message = "";
  let url ={ "open-url" : `openjd://virtual?params=%7B%20%22category%22:%20%22jump%22,%20%22des%22:%20%22m%22,%20%22url%22:%20%22https%3a%2f%2fcrazy-joy.jd.com%2f%23%2f%22%20%7D`}
